...
public interface AttendanceRecordRepository extends JpaRepository<AttendanceRecordModel, Integer>
{
    ...
    @Modifying
    @Transactional
    @Query("Update AttendanceRecordModel a set a.overTime = :overTime where a.employeeId = :employeeId")
    void updateOverTime(@Param("employeeId") Long employeeId,@Param("overTime") double overTime);
